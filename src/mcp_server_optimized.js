...
class OptimizedDeboMCPServer {
  ...
  async init() {
    ...
    Optimized Enterprise AI System v3.1.0`);
      ...
      // Initialize terminal command handler with optimized services
      this.terminalHandler = new TerminalCommandHandler(
        this.services.orchestrator, 
        this.services.database.tasks
      );
      logger.info('✅ Terminal command handler initialized');
  ...
  setupWebSocketTestClient() {
    if (this.websocketIntegration && this.websocketIntegration.app) {
      this.websocketIntegration.app.get('/test', (req, res) => {
        const testHtml = createTestClient('ws://localhost:3001');
        res.send(testHtml);
      });
      
      logger.info('🌐 WebSocket test client available at http://localhost:3001/test');
    }
  }
    ...
    const shutdown = async (signal) => {
      ...
      try {
        ...
        if (this.websocketIntegration) {
          await this.websocketIntegration.close();
        }
